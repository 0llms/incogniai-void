...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Void, please download the latest version! [Void Editor](https://chat.hsrprojects.org/download-beta)!'
	...
	if (res?.action) {
		...
		if (res.action === 'reinstall') {
			primary.push({
				...
				run: () => {
					...
					window.open('https://chat.hsrprojects.org/download-beta')
				}
			})
		}
		...
		primary.push({
			...
			run: () => {
				const { window } = dom.getActiveWindow()
				window.open('https://chat.hsrprojects.org/')
			}
		})
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Void Error: There was an error checking for updates. If this persists, please get in touch or reinstall Void [here](https://chat.hsrprojects.org/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'incogniAI: Check for Updates'),
		});
	}
