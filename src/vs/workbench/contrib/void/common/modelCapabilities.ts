...
export const defaultProviderSettings = {
	incogniAI: {
		endpoint: 'https://chat.hsrprojects.org/api/v1',
		apiKey: '',
	},
	anthropic: {
		apiKey: '',
...
const extensiveModelOptionsFallback: VoidStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {
	...
	if (lower.includes('nemotron')) return toFallback(openSourceModelOptions_assumingOAICompat, 'qwen3')
	if (lower.includes('gpt-oss') || lower.includes('oss')) return toFallback(openSourceModelOptions_assumingOAICompat, 'qwen3')
...
const incogniAISettings: VoidStaticProviderInfo = {
	modelOptions: {},
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName),
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
