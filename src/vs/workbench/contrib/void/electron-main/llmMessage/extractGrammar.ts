...
export const extractXMLToolsWrapper = (
	...
	if (!tools || tools.length === 0) return { newOnText: onText, newOnFinalMessage: onFinalMessage }
	...
	const newOnText: OnText = (params) => {
		...
		if (foundOpenTag === null) {
			...
			const fullTagMatch = findIndexOfAny(newFullText, toolOpenTags)
			if (fullTagMatch !== null) {
				const [idx, toolTag] = fullTagMatch
				const toolName = toolTag.substring(1, toolTag.length - 1) as ToolName
				const idxInTrueFullText = trueFullText.indexOf(toolTag)
				foundOpenTag = { idx: idxInTrueFullText !== -1 ? idxInTrueFullText : fullText.length + idx, toolName }

				fullText += newFullText.substring(0, idx)
				openToolTagBuffer = ''
			}
			else {
				const isPartial = findPartiallyWrittenToolTagAtEnd(newFullText, toolOpenTags)
				if (isPartial && isPartial[0] !== isPartial[1]) {
					openToolTagBuffer = newFullText
				}
				else {
