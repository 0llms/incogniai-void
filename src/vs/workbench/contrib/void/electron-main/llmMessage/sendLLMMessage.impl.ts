...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referer': 'https://chat.hsrprojects.org', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'incogniAI', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
	...
	else if (providerName === 'incogniAI') {
		const thisConfig = settingsOfProvider[providerName]
		const endpoint = thisConfig?.endpoint || 'https://chat.hsrprojects.org/api/v1'
		const apiKey = thisConfig?.apiKey || 'noop'
		return new OpenAI({
			baseURL: endpoint,
			apiKey: apiKey,
			defaultHeaders: {
				'Authorization': `Bearer ${apiKey}`,
				'X-Title': 'incogniAI IDE',
			},
			...commonPayloadOpts
		})
	}
	else throw new Error(`incogniAI providerName was invalid: ${providerName}.`)
}
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, chatMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	...
	// always parse out tool results if XML tags are streamed by LLM models
		...
		.then(async response => {
			...
			try {
				...
				for await (const chunk of response) {
					...
					// message text
					const newText = delta?.content ?? delta?.text ?? (choice as any)?.text ?? ''
					...
					} else if (nameOfReasoningFieldInDelta && delta?.[nameOfReasoningFieldInDelta]) {
						newReasoning = delta[nameOfReasoningFieldInDelta]
					}
				...
				console.warn('Stream processing warning:', streamErr)
			}

			// on final
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'incogniAI: Response from model was empty.', fullError: null })
			}
			else {
